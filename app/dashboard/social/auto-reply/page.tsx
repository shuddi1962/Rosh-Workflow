...
export default function SocialAutoReplyPage() {
  ...
  if (loading) return <div className="p-6 text-text-muted flex items-center gap-3"><Loader2 className="w-5 h-5 animate-spin" />Loading auto-reply settings...</div>

  return (
    <div className="bg-bg-void min-h-screen">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-text-primary">Social Auto-Reply</h1>
        <p className="text-sm text-text-muted mt-1">AI-powered automatic responses across all connected platforms</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-xl border border-border-subtle p-5">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-text-muted">Replies Today</span>
          </div>
          <p className="text-2xl font-bold text-text-primary font-mono">{totalReplies}</p>
        </div>
        <div className="bg-white rounded-xl border border-border-subtle p-5">
          <div className="flex items-center gap-2 mb-2">
            <UserPlus className="w-4 h-4 text-green-600" />
            <span className="text-sm text-text-muted">Leads Created</span>
          </div>
          <p className="text-2xl font-bold text-text-primary font-mono">{totalLeads}</p>
        </div>
        <div className="bg-white rounded-xl border border-border-subtle p-5">
          <div className="flex items-center gap-2 mb-2">
            <Zap className="w-4 h-4 text-purple-600" />
            <span className="text-sm text-text-muted">Active Triggers</span>
          </div>
          <p className="text-2xl font-bold text-text-primary font-mono">{activeTriggers}</p>
        </div>
        <div className="bg-white rounded-xl border border-border-subtle p-5">
          <div className="flex items-center gap-2 mb-2">
      ...
      {activeSection === 'platforms' && (
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-border-subtle p-6">
            <h3 className="text-lg font-semibold text-text-primary mb-4">Platform Connections</h3>
            <div className="space-y-3">
              {platforms.map(platform => (
                <div key={platform.id} className="flex items-center justify-between p-4 border border-border-ghost rounded-lg hover:bg-bg-surface transition-colors">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-sm text-text-muted">
                        {platform.isConnected ? (
                          ...
                          <span className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-text-muted/50" /> Not connected</span>
                        )}
                        {platform.lastEvent && <span className="text-xs text-text-muted">| Last event: {platform.lastEvent}</span>}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="hidden sm:flex items-center gap-4 text-xs text-text-muted">
                      {platform.triggers.slice(0, 3).map(t => (
                        <span key={t} className={`px-2 py-0.5 rounded-full ${platform.isEnabled ? 'bg-accent-emerald/10 text-accent-emerald' : 'bg-bg-elevated text-text-muted'}`}>
                    ...
                    <button
                      ...
                      className={`relative w-11 h-6 rounded-full transition-colors ${platform.isEnabled ? 'bg-accent-primary' : 'bg-text-muted/30'}`}
                    >
      ...
      {activeSection === 'keywords' && (
        <div className="space-y-4">
          {/* Test Bar */}
          <div className="bg-white rounded-xl border border-border-subtle p-6">
            <h3 className="text-lg font-semibold text-text-primary mb-3">Test Auto-Reply</h3>
            <div className="flex gap-3">
              <input
                ...
                className="flex-1 px-3 py-2 border border-border-subtle rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
              />
            ...
            {testResult && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-4 p-4 bg-bg-surface rounded-lg border border-border-subtle">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div><span className="text-text-muted">Intent:</span> <span className="font-medium text-text-primary">{(testResult.intent as string) || 'N/A'}</span></div>
                  <div><span className="text-text-muted">Sentiment:</span> <span className="font-medium">{(testResult.sentiment as string) || 'N/A'}</span></div>
                  <div><span className="text-text-muted">Priority:</span> <span className="font-medium">{(testResult.priority as string) || 'N/A'}</span></div>
                  <div><span className="text-text-muted">Create Lead:</span> <span className="font-medium">{testResult.should_create_lead ? 'Yes' : 'No'}</span></div>
                </div>
                {(testResult.reply_text as string) && (
                  <div className="mt-3 p-3 bg-white rounded-lg border border-border-subtle">
                    <span className="text-xs text-text-muted block mb-1">Reply:</span>
                    <p className="text-sm text-text-primary">{testResult.reply_text as string}</p>
                  </div>
                )}
              </motion.div>
            )}
          ...
          <div className="bg-white rounded-xl border border-border-subtle p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-text-primary">Keyword Triggers (Instant, No AI Cost)</h3>
              <button
            ...
            <div className="space-y-2">
              {keywords.map(kw => (
                <div key={kw.id} className={`flex items-center justify-between p-3 rounded-lg border transition-colors ${kw.is_active ? 'border-border-subtle bg-white' : 'border-border-ghost bg-bg-surface opacity-60'}`}>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      {kw.is_active ? <CheckCircle className="w-4 h-4 text-green-600" /> : <XCircle className="w-4 h-4 text-text-muted" />}
                      <h4 className="font-medium text-text-primary text-sm truncate">{kw.keywords.join(', ')}</h4>
                    </div>
                    <p className="text-xs text-text-muted mt-0.5 ml-6">Platform: {kw.platform} | Action: {kw.action || 'Reply only'} | Fired: {kw.fire_count} times</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => toggleKeyword(kw.id)} className={`relative w-9 h-5 rounded-full transition-colors ${kw.is_active ? 'bg-accent-emerald' : 'bg-text-muted/30'}`}>
                    ...
                    <button onClick={() => { setEditingKeyword(kw); setShowKeywordEditor(true) }} className="p-1.5 hover:bg-bg-surface rounded"><Edit3 className="w-3.5 h-3.5 text-text-muted" /></button>
                    <button onClick={() => deleteKeyword(kw.id)} className="p-1.5 hover:bg-accent-red/10 rounded"><Trash2 className="w-3.5 h-3.5 text-accent-red" /></button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white rounded-xl border border-border-subtle p-6">
            ...
            <p className="text-sm text-text-muted mb-4">Auto-reply stops and team is notified when:</p>
          <div className="space-y-3">
            {[
              ...
              <label key={key} className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="h-4 w-4 text-blue-600 rounded border-border-subtle focus:ring-blue-500"
                />
          ...
          <div className="mt-4 pt-4 border-t border-border-subtle">
            <p className="text-sm font-medium text-text-secondary mb-2">Notify via:</p>
            <div className="flex gap-4">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm text-text-secondary">SMS to 08109522432</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
      ...
      {activeSection === 'history' && (
        <div className="bg-white rounded-xl border border-border-subtle p-6">
          <h3 className="text-lg font-semibold text-text-primary mb-4">Recent Auto-Replies</h3>
          {logs.length === 0 ? (
            ...
            <div className="space-y-3">
              {logs.slice(0, 20).map(log => (
                <div key={log.id} className="p-4 border border-border-ghost rounded-lg hover:bg-bg-surface">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-lg">{AUTO_REPLY_PLATFORMS.find(p => p.id === log.platform)?.icon || '💬'}</span>
                      <div>
                        <p className="text-sm font-medium text-text-primary">{log.platform} — {log.trigger}</p>
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`px-2 py-0.5 rounded-full text-xs ${
                        ...
                        log.sentiment === 'negative' ? 'bg-accent-red/20 text-accent-red' : 'bg-bg-elevated text-text-secondary'
                      }`}>{log.sentiment}</span>
                      {log.leadCreated && <span className="px-2 py-0.5 rounded-full text-xs bg-blue-100 text-blue-700">Lead Created</span>}
                    </div>
                  </div>
                  <p className="text-xs text-text-secondary mt-2 ml-7">Incoming: "{log.incoming}"</p>
                  <p className="text-xs text-text-primary mt-1 ml-7">Reply: "{log.reply}"</p>
                </div>
              ))}
            </div>
          )}
      ...
      {activeSection === 'settings' && (
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-border-subtle p-6">
            <h3 className="text-lg font-semibold text-text-primary mb-4">Global Settings</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-bg-surface rounded-lg">
                <div>
                  <p className="font-medium text-text-primary">Master Kill Switch</p>
                  <p className="text-sm text-text-muted">Pause all auto-replies across all platforms</p>
                </div>
                <button
              ...
              <div>
                <label className="block text-sm font-medium text-text-secondary mb-1.5">Response Delay</label>
                <select
                  value={globalSettings.responseDelay}
                  onChange={e => setGlobalSettings(prev => ({ ...prev, responseDelay: e.target.value }))}
                  className="w-full px-3 py-2 border border-border-subtle rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                >
              ...
              <div>
                <label className="block text-sm font-medium text-text-secondary mb-1.5">AI Model</label>
                <select
              ...
              <div className="space-y-3 pt-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm text-text-secondary">Auto-create CRM lead from social interactions</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm text-text-secondary">Notify team for negative sentiment</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm text-text-secondary">Notify team for ORDER_INTENT detected</span>
                </label>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-border-subtle p-6">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between p-3 bg-bg-surface rounded-lg">
                <span className="text-text-secondary">Keyword replies</span>
                <span className="font-medium text-green-600">FREE (no AI cost)</span>
              </div>
              <div className="flex justify-between p-3 bg-bg-surface rounded-lg">
                <span className="text-text-secondary">AI-powered replies</span>
                <span className="font-medium">~$0.002 per reply (Claude)</span>
              </div>
              <div className="flex justify-between p-3 bg-bg-surface rounded-lg">
                <span className="text-text-secondary">Daily AI budget</span>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="p-6 border-b border-border-subtle">
          <h2 className="text-xl font-semibold text-text-primary">{keyword ? 'Edit Keyword Trigger' : 'Add Keyword Trigger'}</h2>
        </div>
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-text-secondary mb-1.5">Keywords</label>
            <div className="flex gap-2 mb-2">
              <input type="text" value={keywordInput} onChange={e => setKeywordInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addKeyword())} placeholder="Type keyword and press Enter" className="flex-1 px-3 py-2 border border-border-subtle rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
          ...
          <div>
            ...
            <select value={formData.platform} onChange={e => setFormData(prev => ({ ...prev, platform: e.target.value }))} className="w-full px-3 py-2 border border-border-subtle rounded-lg text-sm">
              <option value="all">All Platforms</option>
              {AUTO_REPLY_PLATFORMS.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-text-secondary mb-1.5">Reply Template (WhatsApp)</label>
            <textarea value={formData.reply_template.whatsapp || ''} onChange={e => setFormData(prev => ({ ...prev, reply_template: { ...prev.reply_template, whatsapp: e.target.value } }))} rows={4} className="w-full px-3 py-2 border border-border-subtle rounded-lg text-sm" placeholder="Reply message..." />
          </div>
          <div>
            <label className="block text-sm font-medium text-text-secondary mb-1.5">Action</label>
            <select value={formData.action} onChange={e => setFormData(prev => ({ ...prev, action: e.target.value }))} className="w-full px-3 py-2 border border-border-subtle rounded-lg text-sm">
          ...
          <div>
            <label className="block text-sm font-medium text-text-secondary mb-1.5">Tag Product</label>
            <input type="text" value={formData.tag_product} onChange={e => setFormData(prev => ({ ...prev, tag_product: e.target.value }))} className="w-full px-3 py-2 border border-border-subtle rounded-lg text-sm" placeholder="e.g., cctv, solar, marine" />
          </div>
        </div>
        <div className="p-6 border-t border-border-subtle flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 text-sm text-text-secondary border border-border-subtle rounded-lg hover:bg-bg-surface">Cancel</button>
