...
export default function VideoStudioPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="font-clash text-3xl font-bold text-gray-900">Video Creation Studio</h1>
        <p className="text-gray-600 mt-1">Create multi-scene, cinematic videos for marketing</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <h2 className="font-semibold text-gray-900 mb-4">Video Settings</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              <div>
                <label className="block text-sm text-gray-600 mb-1">Division</label>
                <select value={selectedDivision} onChange={e => setSelectedDivision(e.target.value)} className="w-full p-2 border border-gray-200 rounded-lg text-sm">
              ...
              <div>
                ...
                <select value={selectedType} onChange={e => setSelectedType(e.target.value)} className="w-full p-2 border border-gray-200 rounded-lg text-sm">
                  {VIDEO_TYPES.map(t => (
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="font-semibold text-gray-900 flex items-center gap-2">
                <Film className="w-5 h-5 text-purple-600" /> Story Builder
              </h2>
              <div className="flex gap-2">
                <button className="px-3 py-1.5 border border-gray-200 rounded-lg text-sm hover:bg-gray-50">+ Add Scene</button>
            ...
            <div className="space-y-3">
              {SCENES.map((scene, i) => (
                <div key={scene.id} className={`flex items-center gap-3 p-3 rounded-lg border ${
                  scene.status === 'ready' ? 'border-green-200 bg-green-50' :
                  ...
                  <div className="flex items-center gap-2">
                    {scene.status === 'ready' && <span className="text-xs text-green-600 font-medium">✓ Ready</span>}
                    {scene.status === 'generating' && (
                      <span className="text-xs text-amber-600 font-medium flex items-center gap-1">
                        <Loader2 className="w-3 h-3 animate-spin" /> Generating
                      </span>
                    )}
                    {scene.status === 'queued' && <span className="text-xs text-gray-400">Queued</span>}
                    {scene.status === 'ready' && (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <h3 className="font-semibold text-gray-900 mb-4">Audio</h3>
            <div className="space-y-3">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <h3 className="font-semibold text-gray-900 mb-4">Estimate</h3>
            ...
            <button
              ...
              disabled={generating}
              className="w-full mt-4 px-4 py-2.5 bg-purple-600 text-white rounded-lg hover:bg-purple-700 text-sm flex items-center justify-center gap-2 disabled:opacity-50"
            >
