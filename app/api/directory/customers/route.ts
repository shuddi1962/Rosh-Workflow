...
export async function GET(request: Request) {
  ...
  try {
    ...
    const { data, error, setup_required, setup_hint } = await db.from('customers').select('*').order('name', { ascending: true }).limit(500)
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    if (setup_required) {
      return NextResponse.json({ customers: [], count: 0, setup_required: true, setup_hint })
    }
