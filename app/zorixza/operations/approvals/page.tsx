...
export default function ZorixzaApprovalsPage() {
  ...
  const rows = asArray<Approval>(data, ['approvals']).filter(
    (a) => filter === 'all' || String(a.status) === filter
  );

  const columns: ZxColumn<Approval>[] = [
    { key: 'ref', label: 'Record', render: (a) => (
    ...
    { key: 'by', label: 'Requested by', render: (a) => <span className="text-slate-500">{String(a.requested_by_name ?? '—')}</span> },
    { key: 'at', label: 'Raised', render: (a) => <span className="text-slate-500 whitespace-nowrap">{String(a.created_at || '').slice(0, 10)}</span> },
  ...
  if (loading) return <ZxLoading label="Loading approval inbox…" />;
  if (error) return <ZxError message={error} onRetry={reload} />;

  return (
    <ZxSection title="Approval inbox" hint="Approve · return · reject — every decision is audited and notifies the requester">
      <div className="flex gap-1.5 mb-4">
        {['pending', 'approved', 'returned', 'rejected', 'all'].map((s) => (
          <button
            key={s}
            onClick={() => setFilter(s)}
            className={`px-3 py-1.5 rounded-lg text-xs font-bold capitalize transition ${filter === s ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}
          >
            {s}
          </button>
        ))}
      ...
      <ZxTable<Approval>
        ...
        searchPlaceholder="Search record, type, requester…"
        emptyTitle={filter === 'pending' ? 'Inbox clear' : 'Nothing in this state'}
        emptyHint="Quotations, POs, invoices, stock adjustments, documents and leave all queue here."
      />
      {target && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          <form onSubmit={decide} className="relative bg-white rounded-2xl p-5 w-full max-w-md shadow-2xl">
            <p className="text-[11px] font-extrabold uppercase tracking-widest text-slate-400">Decide approval</p>
