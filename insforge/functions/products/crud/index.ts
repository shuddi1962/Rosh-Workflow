export default async function(req: Request) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const resp = await fetch(`${url}/rest/v1/products${query}`, {
    ...
    if (req.method === 'POST') {
      ...
      const resp = await fetch(`${url}/rest/v1/products`, {
        ...
        body: JSON.stringify({ ...data, created_at: new Date().toISOString(), updated_at: new Date().toISOString() })
      });
      return new Response(JSON.stringify(await resp.json()), { headers });
    }
    ...
    if (req.method === 'PUT') {
      ...
      const resp = await fetch(`${url}/rest/v1/products?id=eq.${id}`, {
        method: 'PATCH',
        headers: { 'apikey': key, 'Authorization': `Bearer ${key}`, 'Content-Type': 'application/json', 'Prefer': 'return=representation' },
    ...
    if (req.method === 'DELETE') {
      ...
      await fetch(`${url}/rest/v1/products?id=eq.${id}`, {
        method: 'DELETE',
        headers: { 'apikey': key, 'Authorization': `Bearer ${key}` }
      });
