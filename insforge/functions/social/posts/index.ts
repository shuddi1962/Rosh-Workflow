export default async function(req: Request) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const resp = await fetch(`${url}/rest/v1/social_posts${query}&order=scheduled_at.asc`, {
    ...
    if (req.method === 'PUT') {
      ...
      const resp = await fetch(`${url}/rest/v1/social_posts?id=eq.${id}`, {
        method: 'PATCH',
    ...
    if (req.method === 'DELETE') {
      ...
      await fetch(`${url}/rest/v1/social_posts?id=eq.${id}`, {
        method: 'DELETE',
        headers: { 'apikey': key, 'Authorization': `Bearer ${key}` }
      });
