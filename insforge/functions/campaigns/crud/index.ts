export default async function(req: Request) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const resp = await fetch(`${url}/rest/v1/campaigns${query}&order=created_at.desc`, {
    ...
    if (req.method === 'POST') {
      ...
      const resp = await fetch(`${url}/rest/v1/campaigns`, {
        method: 'POST',
    ...
    if (req.method === 'PUT') {
      ...
      const resp = await fetch(`${url}/rest/v1/campaigns?id=eq.${id}`, {
        method: 'PATCH',
    ...
    if (req.method === 'DELETE') {
      ...
      await fetch(`${url}/rest/v1/campaigns?id=eq.${id}`, {
        method: 'DELETE',
        headers: { 'apikey': key, 'Authorization': `Bearer ${key}` }
      });
